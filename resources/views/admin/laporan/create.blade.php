...
@section('title', 'Buat Rekap Laporan Bulanan — Admin')
@section('page-title', 'Buat Rekap Laporan Bulanan')
...
<div style="margin-bottom:20px;">
    <a href="{{ route('admin.laporan.index', ['kelas_id' => $siswa->kelas_id, 'bulan' => $bulan, 'tahun' => $tahun]) }}" class="btn btn-secondary" style="text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
...
<form method="POST" action="{{ route('admin.laporan.store') }}">
    ...
    @php
        $aspeks = [
            'agama_moral' => ['Agama & Moral', 'nilai_agama_moral', 'catatan_agama_moral', '#7C3AED'],
            'motorik_kasar' => ['Motorik Kasar', 'motorik_kasar', 'catatan_motorik_kasar', '#EF4444'],
            'motorik_halus' => ['Motorik Halus', 'motorik_halus', 'catatan_motorik_halus', '#F59E0B'],
            'kognitif' => ['Kognitif', 'kognitif', 'catatan_kognitif', '#10B981'],
            'bahasa' => ['Bahasa', 'bahasa', 'catatan_bahasa', '#3B82F6'],
            'sosial_emosional' => ['Sosial Emosional', 'sosial_emosional', 'catatan_sosial_emosional', '#EC4899'],
            'seni' => ['Seni', 'seni', 'catatan_seni', '#8B5CF6']
        ];
            ...
            <div class="card-body" style="padding:20px;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:24px;">
                    ...
                    <div class="form-group" style="margin:0;">
                        ...
                        <textarea id="rekap_{{ $key }}" name="rekap_{{ $key }}" class="form-control" style="width:100%; height:220px; font-size:0.85rem; line-height:1.5; resize:none;" placeholder="Tuliskan rangkuman capaian aspek {{ $info[0] }} murid untuk bulan ini berdasarkan catatan guru..." required>{{ old('rekap_'.$key) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
