...
<div class="card">
    <div class="card-header">
        ...
        <a href="{{ route('admin.siswa.create') }}" class="btn btn-primary btn-sm" style="display:inline-flex; align-items:center; gap:6px; font-weight:700;">
                ...
                <tr>
                    ...
                    <td style="white-space:nowrap">
                        ...
                        <form method="POST" action="{{ route('admin.siswa.toggle-aktif', $s) }}" style="display:inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="btn btn-sm {{ $s->is_aktif ? 'btn-secondary' : 'btn-success' }}"
                                    title="{{ $s->is_aktif ? 'Nonaktifkan Siswa' : 'Aktifkan Siswa' }}">
                                @if($s->is_aktif)
                        ...
                        <form method="POST" action="{{ route('admin.siswa.destroy', $s) }}" style="display:inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data siswa {{ $s->nama }} secara permanen?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-sm" style="background:#FEE2E2; color:#EF4444; border:1px solid #FCA5A5;" title="Hapus Permanen">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:13px;height:13px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                            </button>
                        </form>
                    </td>
                </tr>
